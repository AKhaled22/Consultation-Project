...
exports.viewReferee = async (req, res) => {
    ...
    const refreesname=referees.map((referee)=>(
        referee.name
    ));
    res.status(200).json({
        refreesname,
  ...
  exports.viewStadium = async (req, res) => {
    ...
    const stadiumname=stadium.map((stad)=>(
        stad.name
    ));
    res.status(200).json({
        stadiumname,
  ...
  exports.creatematch= async (req, res) => {
    console.log("hello");
    const { HomeTeam, AwayTeam, MatchVenue,date,time,MainReferee,Linesman1,Linesman2,Ticketprice} = req.body;
